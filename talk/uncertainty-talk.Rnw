...
Technically, we would define $p$ as the underlying true probability of getting H in a single toss.
...
# Do for tossed coins:
like_fig <- function(choice)
    {
    nvec = c(1, 2, 5,20,100)			# n tosses
...
**repetition of below?: Similarly, we have fisheries data and want to infer parameters and  make other
calculations, essentially to calculate
...
\begin{frame}
\frametitle{pasted in** Bayesian figures*** }
...
**So we end up with, say, 1,000 representations of reality. Show one from
hake. these are what he have but then can aggregate to show boxplots.
