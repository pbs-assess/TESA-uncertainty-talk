...
\institute{Pacific Biological Station, Nanaimo, BC
  [** add a photo of PBS, or
  a fish, then finalise spacing]}
\date{{\footnotesize SRG meeting -- \Sexpr{assess.yr}}}
\subtitle{\small Lecture 5 of TESA's Introduction to Stock Assessment WebEx series.}
...
\bi
  ...
  \item Types of probabilities we can calculate in assessments
  \item .....
  \item
  \item
  \item
...
\bi
  \item Research Scientist at Pacific Biological Station since 2007
  \item Previously worked at Bedford Institute of Oceanography \& Dalhousie
...
\medskip
%\hfill
~~~\includegraphics[width=12cm]{images/notation-header.png}
% \vskip10pt
\end{frame}
...
\begin{frame}     % AME - not sure if can just use \frame{} everywhere like the
                  %  above slide
\frametitle{Basics **just have one row for first}

\begin{tabular}{lrrrr}
  & Fraction & Decimal & Percentage & Odds \\
  & & & & \\
Probability that a tossed coin lands heads: & $\dfrac{1}{2}$ & \alert<2-3>{0.5} & \textcolor<3>{blue}{50\%} & 1 in
                                                                             2 \\
  & & & & \\
Probability that a rolled die gives a six: & $\dfrac{1}{6}$ & \alert<2-3>{0.167} & \textcolor<3>{blue}{16.7\%} & 1 in
                                                                            6
\end{tabular}
...
\onslide<2-3>\alert{~~~~~~~~Easier to use in calculations}\\

~\\

\onslide<3>\textcolor{blue}{~~~~~~~~Maybe more easily understood by stakeholders}
...
\begin{tabular}{ll}
  & Examples \\
  & \\
P(impossible event) = 0 & P(standard die gives a 7) \\
  & \\
P(definite event) = 1 & P(everyone's children got too much candy\\
  &                       ~~~~trick-or-treating last night) \\
  & \\
Everything else is between 0 and 1 & P(it will rain tomorrow at a given location)\\
                                   & P(fish stock is currently in the healthy zone)
...
Probabilities of all possible events add up to 1.

P(heads) = 0.5

P(tails) = 0.5

P(heads) $+$ P(tails) = 1~~ (the coin must lands on heads or tails)
...
\begin{frame}
\frametitle{What about the other way round?}
...
600 t is above average recent catch of 547 t.
...
\begin{frame}
\frametitle{How to fisheries managers use information to make decisions?}
...
#      format = "latex",
#      booktabs = TRUE)
print(xtable(as.data.frame(df)), include.rownames = FALSE)
# kable(df)
@
...
print(xtable(as.data.frame(df2)), include.rownames = FALSE)
@
**mention Tammi - looked at in a different way that I had thought. If quota is 0
or ** it's not really changing the probability much.
...
\begin{frame}
\frametitle{**Try kable again}

Nope, get conflict with xcolor
...
Bet on one number in roulette. Probability of winning is the same whether you
bet \$5 or \$100,000.

But betting \$100,000 intuitively sounds riskier -- that's because the negative
outcome (of losing) is more serious than just losing \$5.
