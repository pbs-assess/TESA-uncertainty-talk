...
For real data, this would involve fitting a model:
...
lines(xvals, yvals*500*6, col="red", lwd=3) # 6 is width/breaks
@
...
<<hist-simp-6, fig.height=2.0>>=
par(mai=cmai)
xvalslog = seq(0, 160, length=2000)
