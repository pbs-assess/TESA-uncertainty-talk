...
\begin{minipage}{12cm}
\bi
  \item Demonstrate ways that stock assessment scientists can include
    uncertainty in advice
  \item \textcolor{blue}{Explain how to interpret such uncertainty}
  \item Relate the concepts to fairly simple examples
  \item \textcolor{blue}{Describe background to some of the methods used}
  \item Caveats and potential pitfalls
\ei
\end{minipage}
\end{frame}
...
A die rolls can only produce \alert{discrete} values: 1, 2, 3, 4, 5 or 6.
...
Typically display such data as histogram, necessarily bins the data **do 30:
% hist(lynx, breaks=30)
% hist(lynx, breaks=100)


**dumped: But many measured values are \alert{continuous} -- they can take any value from
...
However, for continuous variables, we cannot say P(this fish weighs 257~g).

~\\

No fish will weigh \alert{exactly} 257~g. Therefore

~~P(this fish weighs 257~g) = 0
...
**Draw cts, shade in, based on plot(thetavec).
...
**In practice can think of as representing a histogram of results.

**Then do mean, median, mode. And quantiles.
...
For example, based on projections, show the
probabilities of being in the healthy zone each year based on different
...
Can show decimals in tables (less clutter) and percentages in text which can
read more naturally for stakeholders (e.g. Pacific Herring).
...
e.g. P(rains tomorrow | rains today)
...
Likelihood of \textcolor{red}{hypothesis (coin is fair)} given the
\textcolor{blue}{data (number of heads)}.


%We have data and want to infer whether the coin is fair (whether the underlying probability
%of getting a H, $p$, is 0.5).

%~\\

%~~P(\textcolor{red}{$p = 0.5$} | \textcolor{blue}{data}).
...
Similarly, we have fisheries data and want to infer parameters and  make other
calculations, essentially to calculate things such as
...
~~i.e.~P(\textcolor{red}{hyopthesis} | \textcolor{blue}{data}).

~\\

\pause

So likelihood has a specific technical meaning (so please don't say `likelihood' when you simply mean `probability').
...
\begin{frame}
\frametitle{Biomass index from acoustic Pacific Hake survey}

\includegraphics[width=8cm, center]{images/hake-acoustic-fit.png}
\vspace{-10mm}\\
...
So we do not just show median, also need to show the uncertainty.
...
\item Model gives \alert{projections} based on assumptions being valid and
  continuing to be valid, not \textcolor{blue}{predictions}.
...
\item One approach (recent Pacific Cod assessment) is to average the decision
  ...
  \alert{`best'} model **See what Rob things ;)
\item **discuss with Rob, Carrie. Risk, could then segway into risk
\ei
