...
\bc
P(stock will be in the critical zone next year if catch stays the same)
\ec
...
Will be different for different stakeholders.
...
So don't say `risk' when you just mean `probability'.
...
Thanks for feedback/comments/emails: Rob Kronlund, Carrie Holt, Jaclyn Cleary, Rob Tadey,
Ian Taylor.
